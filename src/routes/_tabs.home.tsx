...
import { Bell, ChevronLeft, Search, Sparkles } from "lucide-react";
import { Card, Screen, SectionTitle } from "@/components/mobile/Shell";
    ...
    <Screen>
      ...
      <div className="px-5 pt-6 pb-3 flex items-center justify-between">
        ...
        <div className="flex gap-1.5">
          <Link to="/search" className="tap grid h-11 w-11 place-items-center rounded-full bg-surface border border-border">
      ...
      >
        ...
        <div className="mt-4 rounded-2xl bg-white/10 p-3 text-xs backdrop-blur">
          مصروفك المسموح يومياً: <span className="num font-bold">{fmtSAR(perDay)}</span> ر.س
        </div>
      </motion.div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 px-4">
        <Link to="/burnout">
          <Card className="flex flex-col items-center gap-1">
            <Ring
              value={s.burnoutScore} max={100} size={100} stroke={10}
              color={`var(--${tier.color})`}
              track="color-mix(in oklab, var(--muted) 90%, transparent)"
            >
              <div className="text-center">
                <div className="num text-2xl font-black">{s.burnoutScore}</div>
                <div className={`text-[10px] font-bold text-${tier.color}`}>{tier.label}</div>
              </div>
            </Ring>
        ...
        <Card className="mx-4 mt-3 !bg-primary-soft border-primary/20">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="text-xs font-bold text-primary">اكتشاف من الذكاء</div>
